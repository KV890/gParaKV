...
static const char* FLAGS_vlog = nullptr;
...
class Stats {
 ...
 public:
  ...
  void Report(const Slice& name) {
    ...
    fprintf(stdout,
            "%-12s : %11.3f micros/op %ld ops/sec %.3f seconds %d"
            " operations;%s%s\n",
            name.ToString().c_str(), seconds_ * 1e6 / (double)done_,
            (long)throughput, elapsed, done_, (extra.empty() ? "" : " "),
            extra.c_str());
    if (FLAGS_histogram) {
 ...
 public:
  Benchmark()
    ...
    for (const auto& file : files) {
      g_env->DeleteFile(std::string(FLAGS_db) + "/" + file);
    }
    if (!FLAGS_use_existing_db) {
      DestroyDB(FLAGS_db, Options());
    }

    g_env->GetChildren(FLAGS_vlog, &files);
    for (const auto& file : files) {
      g_env->DeleteFile(std::string(FLAGS_vlog) + "/" + file);
    }
  }
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
      ...
      arg[i].thread = new ThreadState(i, 1000);
  ...
  void Open() {
    ...
    Status s = DB::Open(options, FLAGS_db, FLAGS_vlog, &db_);
    if (!s.ok()) {
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_db = argv[i] + 5;
    } else if (strncmp(argv[i], "--vlog=", 7) == 0) {
      FLAGS_vlog = argv[i] + 7;
    } else {
  ...
  leveldb::my_stats.original_value_size = FLAGS_value_size;
  uint32_t encoded_original_value_size;
  if (FLAGS_value_size < 1 << 7) {
    encoded_original_value_size = 1;
  } else if (FLAGS_value_size < 1 << 14) {
    encoded_original_value_size = 2;
  } else {
    encoded_original_value_size = 3;
  }
  // 1044
  leveldb::my_stats.var_key_value_size =
      1 + 16 + 1 + FLAGS_value_size + encoded_original_value_size;
  ...
  leveldb::my_stats.max_num_log = FLAGS_num / max_num_log_item + 1;
