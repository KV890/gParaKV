...
class DBImpl : public DB {
 public:
  ...
  Status GetPtr(const ReadOptions& options, const Slice& key,
                std::string* value);
 ...
 private:
  ...
  void MaybeScheduleCompactionForRead() EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  static void BGWorkForRead(void* db);
  void BackgroundCallForRead();
  void BackgroundCompactionForRead() EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  ...
  Status DoGPUCompactionWork(CompactionState* compact)
      EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  ...
  bool background_gc_scheduled_ = false;
  bool background_migrate_scheduled_ = false;

  const std::string vlog_name_;
